...
class CombinedModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, labels=None):
        # outputs1 = self.model1(input_ids, attention_mask, token_type_ids).hidden_states[-1][:, 0, :]# return the output of first token
        ...
        for i in range(len(output)):
            ...
            # output_pool.append(torch.mean(output[i], dim=1))   # Agg all output of each model
        ...
        # agg_output = torch.sum(normalized_output, dim=1)
        # print("agg_output= ",agg_output.size())
       

        # Pass through feed-forward layer for task 
